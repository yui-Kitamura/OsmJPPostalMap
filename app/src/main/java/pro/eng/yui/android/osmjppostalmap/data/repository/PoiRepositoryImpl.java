...
public class PoiRepositoryImpl implements PoiRepository {
    ...
    @Override
    public LiveData<List<OsmPoi>> getPois(String prefectureName) {
        ...
        List<OsmPoi> poiList = JpPostalUtil.getPoiData(prefectureName);
        if (poiList.isEmpty()) {
            try {
                JpPostalUtil.callOverpass(OverpassQuery.getPostSearchQuery(prefectureName));
            }catch (IOException ioe) {
                errorLiveData.postValue("データの取得に失敗しました: " + ioe.getMessage());
                return poisLiveData;
            }
        }
        ...
        poisLiveData.postValue(poiList);
        return poisLiveData;
    }
    ...
    public void savePoi(OsmPoi poi, String comment, PoiSaveCallback callback) {
        ...
        try {
            ...
            ioe.printStackTrace();
            callback.onError("ChangeSetの登録開始処理に失敗しました。リトライしてください");
            return;
        }
        ...
        try {
            ...
            ioe.printStackTrace();
            callback.onError("入力内容の反映に失敗しました。リトライしてください");
        }
    }
    ...
    public void addPostBox(double lat, double lon, String shape, String branch, String postboxRef, String collectionTimes, String note, PoiSaveCallback callback) {
        ...
        try {
            ...
            callback.onError("通信エラー: " + e.getMessage());
        }
    }
    ...
    public void addNote(double lat, double lon, String text, PoiSaveCallback callback) {
        ...
        JpPostalUtil.createNote(lat, lon, finalNote).enqueue(new Callback<okhttp3.ResponseBody>() {
            @Override
